...
// =========================================================
// TODO: TEMPELKAN KONFIGURASI FIREBASE KAMU DI SINI
// Ganti nilai di dalam firebaseConfig dengan milik proyekmu
// =========================================================
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  // Listener Firebase Auth: Otomatis mendeteksi status login/logout
...
function cacheElements() {
  [
    ...
    'theme-toggle', 'open-history', 'history-count', 'open-settings', 'profile-avatar', 'profile-name',
...
async function initializeApplication() {
  if (!window.indexedDB) {
    console.warn('Browser ini tidak mendukung IndexedDB untuk menyimpan riwayat.');
  } else {
    try {
      await getDatabase();
    } catch (error) {
...
function initializeTheme() {
  var savedTheme = safeStorageGet(THEME_KEY, true);
...
function bindAuthEvents() {
  ...
  el.username.addEventListener('input', function () {
    this.value = this.value.replace(/\s/g, ''); // Hapus spasi untuk email
...
function bindStudioEvents() {
  el['theme-toggle'].addEventListener('click', function () {
    ...
    safeStorageSet(THEME_KEY, isDark ? 'dark' : 'light', true);
  });
...
function setAuthMode(mode) {
  ...
  el['auth-kicker'].innerHTML = '<i></i> ' + (registering ? 'DAFTAR AKUN BARU' : 'SELAMAT DATANG');
  el['auth-heading'].textContent = registering ? 'Buat Akun' : 'Masuk ke Studio';
  el['auth-description'].textContent = registering ? 'Daftar menggunakan email untuk menyimpan riwayat lintas perangkat.' : 'Gunakan email dan password kamu untuk masuk.';
  el['auth-submit-label'].textContent = registering ? 'Daftar & Masuk' : 'Masuk ke Studio';
  el['switch-copy'].textContent = registering ? 'Sudah punya akun?' : 'Belum punya akun?';
  el['switch-auth-mode'].textContent = registering ? 'Masuk' : 'Daftar';
  el.password.autocomplete = registering ? 'new-password' : 'current-password';
  setAuthMessage('');
}

async function submitAuthForm() {
  var email = el.username.value.trim();
  var password = el.password.value;
  var displayName = (el['display-name'].value || email.split('@')[0]).trim();

  // Validasi email sederhana
  if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email)) {
    el.username.setAttribute('aria-invalid', 'true');
    setAuthMessage('Harap masukkan alamat email yang valid.');
    return;
  }
  if (password.length < 6) {
    el.password.setAttribute('aria-invalid', 'true');
    setAuthMessage('Kata sandi minimal terdiri dari 6 karakter.');
    return;
  }
  if (state.authMode === 'register' && displayName.length > 42) {
    setAuthMessage('Nama tampilan maksimal 42 karakter.');
    return;
  }
  ...
  try {
    if (state.authMode === 'register') {
      setAuthMessage('Mendaftarkan akun...', true);
      const userCredential = await createUserWithEmailAndPassword(auth, email, password);
      await updateProfile(userCredential.user, { displayName: displayName });
      // onAuthStateChanged akan otomatis mengalihkan ke Studio
    } else {
      setAuthMessage('Memverifikasi akun...', true);
...
function translateFirebaseError(code) {
  switch (code) {
    case 'auth/email-already-in-use': return 'Email tersebut sudah terdaftar.';
    ...
    case 'auth/network-request-failed': return 'Gagal terhubung ke internet. Periksa koneksimu.';
    case 'auth/too-many-requests': return 'Terlalu banyak percobaan masuk. Coba lagi nanti.';
    default: return 'Gagal memproses. Silakan coba lagi.';
  }
}
...
function populateProfile() {
  ...
  var name = state.currentUser.displayName || state.currentUser.email.split('@')[0];
  ...
  el['settings-username'].textContent = state.currentUser.email;
}
...
async function logout() {
  try {
    ...
    setAuthMessage('Sesi telah berakhir.', true);
  } catch (error) {
...
async function deleteAllLocalData() {
  if (!window.confirm('Hapus riwayat lokal dari browser ini? (Akun Firebase kamu tetap aman).')) return;
  try {
    if (el['settings-dialog'].open) el['settings-dialog'].close();
    if (databasePromise) {
    ...
    setStatus('Riwayat lokal File Studio telah dihapus.', 'success');
  } catch (error) {
    console.error(error);
    setStatus('Data lokal belum dapat dihapus.', 'error');
  }
}
...
function addFiles(newFiles) {
  ...
  newFiles.forEach(function (file) {
    ...
    else if (file.size > MAX_FILE_SIZE) rejected.push(file.name + ' melebihi batas 80 MB.');
...
// ------------------------------------------------------------------
// SISTEM RIWAYAT LOKAL (Disimpan berdasarkan UID dari Firebase)
// ------------------------------------------------------------------
async function saveHistory(outputs) {
  if (!state.currentUser) return;
  try {
    await idbPut('history', {
      profileId: state.currentUser.uid, tool: state.selectedTool, inputCount: state.files.length,
...
async function refreshHistoryCount() {
  if (!state.currentUser) return;
  try {
    var entries = await idbGetAllByIndex('history', 'profileId', state.currentUser.uid);
...
async function clearHistory() {
  ...
  try {
    await idbDeleteByIndex('history', 'profileId', state.currentUser.uid);
    await refreshHistoryCount();
    await renderHistory();
...
/* IndexedDB helpers (hanya untuk menyimpan riwayat) */
...
async function idbGetByIndex(storeName, indexName, value) { return idbRequest(storeName, 'readonly', function (store) { return store.index(indexName).get(value); }); }
async function idbGetAllByIndex(storeName, indexName, value) { return idbRequest(storeName, 'readonly', function (store) { return store.index(indexName).getAll(IDBKeyRange.only(value)); }); }

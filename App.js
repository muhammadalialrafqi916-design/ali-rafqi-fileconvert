(function () {
  ...
  async function runConversion() {
    ...
    try {
      ...
      else if (state.selectedTool === 'video-audio') outputs = await convertVideoToAudio();
  ...
  /* Fitur Ekstrak Audio dari Video MP4 (Optimasi Memori & Tanpa Server) */
  async function convertVideoToAudio() {
    var AudioContext = window.AudioContext || window.webkitAudioContext;
    if (!AudioContext) {
      throw new Error('Browser Anda tidak mendukung Web Audio API untuk mengekstrak audio.');
    }

    return processFilesWithRecovery('Mengekstrak audio dari video', async function (file) {
      var objectUrl = null;
      var audioCtx = null;

      try {
        // 1. Pakai ArrayBuffer secara bertahap dan segera bersihkan setelah di-decode
        ...
        var audioBuffer = await new Promise(function (resolve, reject) {
          audioCtx.decodeAudioData(
            ...
            function (err) { reject(new Error('Gagal membaca format audio dari file MP4 ini.')); }
          );
        });

        // Hapus referensi arrayBuffer awal agar memori segera dilepaskan oleh Garbage Collector
        arrayBuffer = null;

        // 2. Konversi ke Blob Audio (Standard WAV/PCM Container yang valid & kompatibel)
        var audioBlob = bufferToAudioBlob(audioBuffer);

        // 3. Bebaskan AudioBuffer dari memori
        audioBuffer = null;

        return [{
          blob: audioBlob,
          filename: cleanFileName(stripExtension(file.name) + '-lagu.wav'),
        ...
        // Pembersihan mutlak Web Audio Context untuk mencegah memory leak
        if (audioCtx) {
  ...
  /* Helper Konversi AudioBuffer ke Blob WAV Asli & Valid */
  function bufferToAudioBlob(abuffer) {
    ...
    setUint32(length - 8);                         // Ukuran file
    setUint32(0x45564157);                         // "WAVE"
    setUint32(0x20746d66);                         // Chunk "fmt "
    setUint32(16);                                 // Panjang chunk (16 untuk PCM)
    setUint16(1);                                  // Format PCM
    setUint16(numOfChan);                          // Jumlah Channel
    setUint32(abuffer.sampleRate);                 // Sample Rate (misal 44100Hz)
    setUint32(abuffer.sampleRate * 2 * numOfChan); // Byte rate
    setUint16(numOfChan * 2);                      // Block align
    setUint16(16);                                 // Bit per sample (16-bit)
    ...
    // Mengembalikan Blob dengan ekstensi audio/wav murni agar tidak terdeteksi corrupt
  ...
  function historyIcon(tool) { return tool === 'images' ? '⌑' : tool === 'pdf-images' ? '▧' : tool === 'images-pdf' ? '▤' : tool === 'merge-pdf' ? '⧉' : tool === 'video-audio' ? '♫' : '≡'; }
  ...
  function extensionFromMime(mime) { return mime === 'image/jpeg' ? 'jpg' : mime === 'image/webp' ? 'webp' : mime === 'application/pdf' ? 'pdf' : mime === 'audio/wav' ? 'wav' : 'png'; }
